  ...
  <div class="container">
    ...
    <div class="info">
      ...
      <div class="right">
        ...
        <van-button :icon=" isHeart ? 'plus': ''" type="primary" :color=" isHeart ? '#4A90E2' : '#bbb'"  @click="changeHeart(list.id)">{{isHeart ? '收藏' : '已收藏' }}</van-button>
      </div>
    </div>
    <!-- 使用van-sticky有问题 -->
    <div class="scro">
      <div class="playlist">音频列表</div>
      <van-button icon="play-circle" plain type="info" hairline round
        @click="toPlayer(0)" >播放全部</van-button>
...
this.index=this.$route.params.index
  ...
  methods: {
    ...
    toPlayer(index) {
      this.$router.push(
        `/player/${this.id}/${index}/${this.categoryId}/${this.index}`
      );
    },
